...
async function ensureWealthOSSchema() {
  ...
  try {
    ...
    await client.query(`
      ...
      ALTER TABLE wealthos_assets ADD COLUMN IF NOT EXISTS currency VARCHAR(10) DEFAULT 'INR';
      ALTER TABLE wealthos_assets ADD COLUMN IF NOT EXISTS fees DECIMAL(18, 6) DEFAULT 0.0;
      ALTER TABLE wealthos_assets ADD COLUMN IF NOT EXISTS taxes DECIMAL(18, 6) DEFAULT 0.0;
      ALTER TABLE wealthos_assets ADD COLUMN IF NOT EXISTS dividend DECIMAL(18, 6) DEFAULT 0.0;
      ALTER TABLE wealthos_assets ADD COLUMN IF NOT EXISTS notes TEXT;
      ALTER TABLE wealthos_assets ADD COLUMN IF NOT EXISTS tags TEXT[] DEFAULT '{}';
      ALTER TABLE wealthos_assets ADD COLUMN IF NOT EXISTS attachments JSONB DEFAULT '[]';
    `);
    ...
    await client.query(`
      ...
      ALTER TABLE wealthos_journal ADD COLUMN IF NOT EXISTS linked_symbol VARCHAR(50);
      ALTER TABLE wealthos_journal ADD COLUMN IF NOT EXISTS target_price DECIMAL(18, 6);
      ALTER TABLE wealthos_journal ADD COLUMN IF NOT EXISTS stop_loss DECIMAL(18, 6);
      ALTER TABLE wealthos_journal ADD COLUMN IF NOT EXISTS confidence_rating INT;
      ALTER TABLE wealthos_journal ADD COLUMN IF NOT EXISTS emotion_check VARCHAR(50);
      ALTER TABLE wealthos_journal ADD COLUMN IF NOT EXISTS status VARCHAR(50) DEFAULT 'thesis_intact';
    `);
    ...
    // 6. Alerts & Automation Rules Table
    await client.query(`
      CREATE TABLE IF NOT EXISTS wealthos_alerts(
      id            UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
      user_id       UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
      alert_type    VARCHAR(50) NOT NULL CHECK(alert_type IN(
        'portfolio_drawdown', 'allocation_drift', 'dividend_credited',
        'goal_behind_schedule', 'target_price', 'earnings_announcement',
        'unusual_volume', 'credit_downgrade', 'tax_loss_harvesting'
      )),
      symbol        VARCHAR(50),
      threshold     DECIMAL(18, 6),
      criteria_desc VARCHAR(255) NOT NULL,
      is_active     BOOLEAN DEFAULT true,
      triggered     BOOLEAN DEFAULT false,
      triggered_at  TIMESTAMPTZ,
      created_at    TIMESTAMPTZ DEFAULT NOW()
    );
      CREATE INDEX IF NOT EXISTS idx_wealthos_alerts_user ON wealthos_alerts(user_id);
    `);
    ...
    for (const u of usersResult.rows) {
      ...
      const assetCountResult = await client.query("SELECT COUNT(*) FROM wealthos_assets WHERE user_id = $1", [userId]);
      const assetCount = parseInt(assetCountResult.rows[0].count);

      if (assetCount === 0) {
        console.log(`🌱 Seeding sample WealthOS data for user: ${ u.username }...`);
    ...
    VALUES($1, $2, $3, $4, $5, $6, $7, $8, $9, 'INR')
             RETURNING id`,
            [userId, a.name, a.asset_class, a.symbol, a.quantity, a.avg_price, a.current_price, a.exchange || null, a.broker || null]
          );
    ...
    VALUES($1, $2, 'buy', $3, $4, $5, $6, $7, NOW() - INTERVAL '6 months', 'Initial position seeding')`,
              [userId, assetId, a.symbol, a.asset_class, a.quantity, a.avg_price, a.quantity * a.avg_price]
            );
          }
        }
    ...
    VALUES($1, $2, $3, $4, $5, $6, $7)`,
            [userId, g.name, g.target_amount, g.current_amount, g.monthly_sip, g.years_remaining, g.probability]
          );
        }
    ...
    VALUES
      ...
      ($1, 'SBI FD Certificate 2026', 'broker_statement', 'sbi_fd_cert.pdf', 'STATE BANK OF INDIA\nTerm Deposit Receipt\nPrincipal: INR 3,00,000\nInterest Rate: 6.5% p.a.\nMaturity Date: 2027-06-20\nMaturity Value: INR 3,20,135\nTax Status: Taxable at Slab Rate.')`,
        ...
        console.log(`✅ WealthOS sample data seeded successfully for user: ${ u.username }.`);
      }

      // Check if alerts already exist for this user
      const alertCountResult = await client.query("SELECT COUNT(*) FROM wealthos_alerts WHERE user_id = $1", [userId]);
      const alertCount = parseInt(alertCountResult.rows[0].count);

      if (alertCount === 0) {
        console.log(`🌱 Seeding sample WealthOS Alerts for user: ${ u.username }...`);
        await client.query(
          `INSERT INTO wealthos_alerts
      (user_id, alert_type, symbol, threshold, criteria_desc, is_active, triggered)
    VALUES
      ($1, 'portfolio_drawdown', NULL, 10.0, 'Portfolio drawdown exceeds 10%', true, false),
      ($1, 'allocation_drift', NULL, 5.0, 'Asset allocation drifts by 5%', true, true),
      ($1, 'dividend_credited', 'TCS', NULL, 'Dividend credited for TCS holding', true, true),
      ($1, 'goal_behind_schedule', 'Retirement', NULL, 'Goal: Retirement falls behind schedule', true, false),
      ($1, 'target_price', 'RELIANCE', 2800.0, 'RELIANCE crosses target price of ₹2,800.00', true, false),
      ($1, 'earnings_announcement', 'TCS', NULL, 'Earnings announcement for TCS', true, false),
      ($1, 'unusual_volume', 'GOLDBEES', 3.0, 'GOLDBEES trading volume exceeds 3x 20-day average', true, false),
      ($1, 'credit_downgrade', 'SBI Fixed Deposit', NULL, 'Credit rating downgrade alert', true, false),
      ($1, 'tax_loss_harvesting', NULL, 10000.0, 'Tax-loss harvesting opportunity exceeds ₹10,000', true, true)`,
          [userId]
        );
      }
    }
